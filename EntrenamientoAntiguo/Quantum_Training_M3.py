...
N_QUBITS = 6
LEARNING_RATE = 0.05
CHECKPOINT_FILE = "checkpoints_hero/checkpoint_m3.pth" 
CSV_FILE = "resultados_entrenamiento_m3.csv" # 🟢 NUEVO: Archivo de salida
...
def qnode(inputs, weights):
    ...
    # 🟢 CAMBIO AL MODELO 3: Circuito Superficial (Shallow Circuit)
    qml.BasicEntanglerLayers(weights, wires=range(N_QUBITS))
    return qml.expval(qml.PauliZ(0))

# 🟢 CAMBIO AL MODELO 3: BasicEntanglerLayers usa la forma (Capas, Qubits)
...
async def train_hero_run():
    ...
    # 🟢 NUEVO: Crear el archivo CSV y escribir los encabezados si empezamos de cero
    if start_epoch == 0 and start_batch_global == 0:
        with open(CSV_FILE, mode='w', newline='') as file:
            writer = csv.writer(file, delimiter=';') # Usamos punto y coma para que Excel europeo lo lea bien
            writer.writerow(["Epoch", "Batch", "Loss", "Grad_Norm", "Weight_Change", "Bias"])

    print(f"\n🚀 INICIANDO HERO RUN M3 ({EPOCHS} Epochs) 🚀")
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        for i in range(0, len(X_train), BATCH_SIZE):
            ...
            for j in range(len(x_batch)):
                ...
                with qml.tape.QuantumTape() as tape:
                    ...
                    qml.BasicEntanglerLayers(weights, wires=range(N_QUBITS)) # 🟢 Modelo 3
            ...
            # 🟢 NUEVO: Añadir línea al archivo Excel/CSV
        ...
        print(f"\n✅ Epoch {epoch+1} Completada | Loss: {avg_loss:.6f} | Tiempo: {duration:.1f}s")
        print("=" * 80)

    await runner.cleanup()
    print("🏆 ¡HERO RUN M3 COMPLETADO!")
